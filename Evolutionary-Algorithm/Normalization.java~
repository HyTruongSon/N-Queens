//Library to resize an matrix image
//Copy right 2012 - 2014, Hy Truong Son
//All rights reserved
...
public class Normalization {
	...
	static int X, Y, u1, v1, u2, v2;
	static int sum[][];
	
	public static int UCLN(int u, int v){
		while ((u > 0) && (v > 0))
			if (u >= v) u %= v; else v %= u;
		if (u > 0) return u;
		return v;
	...
	public static int BCNN(int u, int v){
		int t = UCLN(u, v);
		return (u / t) * v;
	}
	...
	public static void Resize(int f[][], int g[][], int x1, int y1, int x2, int y2){
		...
		u2 = X / x2;
		v2 = Y / y2;
		sum = new int [x1 + 1][y1 + 1];
		for (i = 0; i <= x1; i++) sum[i][0] = 0;
		for (j = 0; j <= y1; j++) sum[0][j] = 0;
		for (i = 1; i <= x1; i++)
			for (j = 1; j <= y1; j++) sum[i][j] = sum[i-1][j] + sum[i][j-1] + f[i-1][j-1] - sum[i-1][j-1];
		for (i = 0; i < x2; i++)
			for (j = 0; j < y2; j++){
				i1 = i * u2;
				j1 = j * v2;
